...
The 2024 DSI approach instead identifies the centre of Mars
via the brightest pixel, and sums all flux within a static aperture around that pixel.
This better captures the true measured flux, albeit without correcting for flux
lost outside of the FIFI-LS field of view.
...
deviate between versions by up to 15%. The figure below displays the difference
between the two response versions for all FIFI-LS filter configurations. Manual
selection of response curves is not supported within the Redux GUI or parameter
file formats, however users may choose to modify the lookup table in
*sofia_redux\sofia_redux\instruments\fifi_ls\data\response_files\response_default.txt*,
pointing to the older response curves if desired. A more detailed investigation
will be provided in future documentation of the SDC flux calibration.
...
.. figure:: images/fifi_fadda_dsi_response_comparison.png
   ...
   PLACEHOLDER IMAGE
   Comparison of Fadda et al. 2023 and DSI 2024 response curves
